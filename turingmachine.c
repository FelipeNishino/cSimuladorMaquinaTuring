...
bool testWord(TuringMachine machine, char* word) {
	// printf("%d\n", strlen(word) * sizeof(char));
	// char* a = malloc(1 + strlen(word) * sizeof(char));
	// a[0] = '-';
	// strncpy(a + 1, word, strlen(word));
	// printf("%s\n", a);
	// strncpy(machine.tape, a, strlen(a));
	...
	while (machine.currentState != machine.nStates) {
		...
		if (strpbrk(machine.inputAlphabet, symbol) == NULL){
			...
			printf("Error: Symbol %c at pos %d not in alphabet\n", machine.tape[machine.headPosition], machine.headPosition);
			return false;
		}
